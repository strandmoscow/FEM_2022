...
constexpr double EPS = 1e-16,CUB= {{ cube }};
double EPS = 1e-16;
double X_BEGIN = {{ min_int }};
double X_END = {{ max_int }};
size_t ELEMS_NUM = {{ elem }};
double L = (X_END - X_BEGIN) / ELEMS_NUM;

constexpr double a = {{ a }}, B = {{ b }}, C = {{ c }}, D = {{ d }}, usl_left = {{ min_value }}, usl_right = {{ max_value }}; // au"+Bu'+Cu+D=0
...
std::vector<double> build_linear_solution(size_t elems_num) {
    ...
    // Учет ГУ {% if min_cond_type == 0 %}
    b.at(0) = usl_left;
    A.at(0).at(0) = 1;
    A.at(0).at(1) = 0;
    {% endif %}{% if min_cond_type == 1 %}
    b.at(0) =  D * L /2. - a*usl_left;
    {% endif %}{% if max_cond_type == 0 %}
    b.at(size - 1) = usl_right;
    A.at(size - 1).at(size - 1) = 1;
    A.at(size - 1).at(size - 2) = 0;
    {% endif %}{% if max_cond_type == 1 %}
